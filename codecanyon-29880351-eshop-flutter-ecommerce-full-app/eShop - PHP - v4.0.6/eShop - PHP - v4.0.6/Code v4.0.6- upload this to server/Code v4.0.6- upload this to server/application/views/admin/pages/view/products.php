                            ...
                            ?>
                                ...
                                ?>
                                    ...
                                    <table class="table table-sm">
                                        ...
                                        <tbody>
                                            <?php
                                            $sale_discount = isset($product_details[0]['sale_discount']) && !empty($product_details[0]['sale_discount']) && ($product_details[0]['sale_discount'] != 0) ? $product_details[0]['sale_discount'] : 0;

                                            $i = 1;
                                            $flag = 0;
                                            //   foreach ($product_details[0]['variants'] as $row) {
                                            foreach ($product_variants as $row) {
                                                $sale_price = strval($row['special_price'] - ($row['special_price'] * ($sale_discount / 100)));
                                                ...
                                                if ($row['special_price'] != null && $row['special_price'] > 0) {
                                                    ...
                                                    $sale_price = strval($row['price'] - ($row['price'] * ($sale_discount / 100)));
                                                    $price = $row['price'];
                                                    //   $strike_off_price = $row['price'];
                                                }
                                            ?>
                                                <tr class='<?= ($row['status'] == 7) ? "table-danger" : (($row['status'] == 0) ? "table-warning" : ""); ?>'>
                                                    ...
                                                    <td><?= isset($product_details[0]['sale_discount']) && !empty($product_details[0]['sale_discount']) && ($product_details[0]['sale_discount'] != 0) ? $sale_price : 0 ?></td>
                                                </tr>
                                            <?php
                                                $i++;
                                                $flag = 0;
