...
class Home_model extends CI_Model
{
    ...
    {
        $count_res = $this->db->select(' COUNT( distinct(p.id)) as `total` ')->join('product_variants', 'product_variants.product_id = p.id', 'left');
        $where = "p.stock_type is  NOT NULL";
        $count_res->where($where);
        ...
        $count_res->where('product_variants.status ', '1');
        $product_count = $count_res->get('products p')->result_array();
        return $product_count[0]['total'];
    }
}
