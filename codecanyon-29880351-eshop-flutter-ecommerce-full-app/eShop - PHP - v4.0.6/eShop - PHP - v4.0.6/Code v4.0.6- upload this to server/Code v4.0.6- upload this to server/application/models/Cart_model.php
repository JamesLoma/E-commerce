...
class Cart_model extends CI_Model
{
    ...
    function get_user_cart($user_id, $is_saved_for_later = 0, $product_variant_id = '')
    {
        ...
        $res =  $q->select('c.*,p.is_prices_inclusive_tax,p.name,p.id,p.image,p.short_description,p.minimum_order_quantity,p.shipping_method,p.pickup_location,p.is_on_sale,p.type,p.sale_discount,
        p.quantity_step_size,p.total_allowed_quantity,pv.price,pv.weight,pv.special_price,pv.id as product_variant_id,tax.percentage as tax_percentage')
            ->order_by('c.id', 'DESC')->get('cart c')->result_array();
            ...
            $res = array_map(function ($d) {
                ...
                if (!empty($sale_dis)) {
                    for ($j = 0; $j < count((array)$sale_dis); $j++) {
                        $sale_amt = get_flash_sale_price($d['price'], $sale_dis[$j]['discount']);
                        $d['special_price'] = $sale_amt;
                    }
                }
                ...
                $d['tax_percentage'] =  (isset($d['tax_percentage']) && !empty($d['tax_percentage'])) ? $d['tax_percentage'] : '0';
